...
def main(args):
    ...
    # model = ConvFormer_s(num_calsses=args.num_classes).to(device)
    model = timm.create_model("mobilevit_s", num_classes=args.num_classes)
    model.to(device)

    pg = [p for p in model.parameters() if p.requires_grad]
    optimizer = optim.AdamW(pg, lr=args.lr, betas=(0.9, 0.999), weight_decay=0.01)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--schedule_epochs', type=int, default=150)
    ...
    parser.add_argument('--data-path', type=str,
                        default="~/autodl-tmp/imagenet/")  # ImageNet/ILSVRC2012"
